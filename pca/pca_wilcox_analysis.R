...
for (RUN in RUNS) {
  ...
  # Perform Kruskal-Wallis test for each axis, separately for Species_Latin and Plumage
  # p_values <- pcl %>%
  #   group_by(axis) %>%
  #   summarise(species = kruskal.test(scaled_value ~ Species_Latin)$p.value,
  #             plumage = kruskal.test(scaled_value ~ Plumage)$p.value) %>%
  #   pivot_longer(!(axis))
